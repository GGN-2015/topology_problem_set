...
1. 验证集合运算 $\cup$ 与 $\cap$ 的分配率及 DeMorgan 定律.
2. 对于所有集合 $A,B,C,D$ 判定下面的那些论断为真. 如果相互蕴含关系不成立，判定有哪一种蕴含关系. 如果等号不成立，判定用包含关系 $\subset$ 或 $\supset$ 代替等号时，相应的表述是否成立？
   ...
   4. $A\subset B$ 且 $A\subset C$ $\Longleftrightarrow$ $A\subset (B \cap C)$.
   5. $A-(A-B)=B$.
   6. $A-(B-A)=A-B$.
